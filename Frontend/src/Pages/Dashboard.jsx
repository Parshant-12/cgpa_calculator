...
import {
  Calculator, Plus, Trophy, X, Info, Target, TrendingUp, Quote,
  HelpCircle, BookOpen, Award, Brain, Library, ArrowUpRight, ChevronDown,
} from "lucide-react";
...
function SemesterCard({ semester, index, onChange, onRemove, formula }) {
      ...
      {!isAverage && (
        <div className="animate-in fade-in slide-in-from-top-2 duration-300">
          <div className="flex items-center justify-between mb-1.5">
            <label className="block text-xs text-slate-400">Total Credits</label>
          ...
          <input
            ...
            className="w-full h-10 px-3 bg-[#060b18] border border-slate-700/50 rounded-lg text-slate-200 text-sm focus:border-brand-primary focus:ring-1 focus:ring-brand-primary/50 outline-none transition-all"
          />
        </div>
      )}
    </div>
...
export default function Dashboard() {
  ...
  const addSemester = () => setSemesters([...semesters, { id: Date.now(), gpa: "", credits: "" }]);
  ...
  return (
    <div className="relative z-10 min-h-screen pb-20 overflow-hidden">
      ...
      <Navbar />
      <main className="max-w-[1300px] mx-auto p-4 md:p-8">
        ...
        <section className="w-full max-w-[1200px] mx-auto p-6 md:p-10 border border-slate-700/40 rounded-3xl bg-[#091022]/90 shadow-[0_0_40px_rgba(0,0,0,0.5)] backdrop-blur-xl relative z-10">
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 mb-10 border-b border-slate-700/40 pb-6">
            <div className="flex gap-4 items-center">
              ...
              <div>
                ...
                <p className="text-slate-500 text-xs mt-0.5">Enter your semester details below</p>
              </div>
            </div>

            {/* UPDATED CUSTOM DROPDOWN */}
            <div className="flex flex-col gap-1.5 w-full lg:w-[450px]">
              <label className="text-xs font-medium text-slate-300">CGPA Formula</label>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-5 mb-10 items-start">
            {semesters.map((sem, idx) => (
              <SemesterCard key={sem.id} semester={sem} index={idx} onChange={updateSemester} onRemove={removeSemester} formula={formula} />
            ))}
          </div>
          ...
          {result && (
            <div className="flex flex-col lg:flex-row items-center justify-between p-8 border border-brand-primary/30 rounded-2xl bg-gradient-to-r from-brand-secondary/10 via-[#0c142a] to-[#0c142a] gap-10 mt-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        {/* Feature & Navigation Links - UPDATED WITH LINK ICONS */}
        <section className="max-w-[1100px] mx-auto mt-12 grid grid-cols-1 md:grid-cols-3 gap-6 relative z-10">

          <Link to="/target-planner" className="relative flex gap-4 items-start p-4 rounded-xl bg-slate-800/20 border border-slate-700/30 hover:border-brand-accent/50 hover:bg-slate-800/40 transition-all cursor-pointer group">
